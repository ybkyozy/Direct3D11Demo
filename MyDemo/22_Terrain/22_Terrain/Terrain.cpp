...
void Terrain::Init(ID3D11Device* device, ID3D11DeviceContext* dc, const InitInfo& initInfo)
{
	...
	// 将高度图分成多个补丁，以便每个补丁都具有CellsPerPatch。
	...
	HR(CreateDDSTextureFromFile(device, mInfo.BlendMapFilename.c_str(), 0, &mBlendMapSRV));
}
...
float Terrain::Average(int i, int j)
{
	...
	// 使用int来允许否定。 如果我们使用 UINT, @ i=0, m=i-1=UINT_MAX
	// 且不发生外循环的迭代。
