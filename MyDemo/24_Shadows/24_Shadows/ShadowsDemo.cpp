...
ShadowsDemo::ShadowsDemo(HINSTANCE hinstance):D3DApp(hinstance)
, mSky(0), mShapesVB(0), mShapesIB(0), mSkullVB(0), mSkullIB(0), mScreenQuadVB(0), mScreenQuadIB(0),
...
void ShadowsDemo::OnMouseDown(WPARAM btnState, int x, int y)
{
	mLastMousePos.x = x;
	mLastMousePos.y = y;
...
void ShadowsDemo::OnMouseUp(WPARAM btnState, int x, int y)
{
	// 不需要继续获得鼠标消息就释放掉
	ReleaseCapture();
}

void ShadowsDemo::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void ShadowsDemo::DrawSceneToShadowMap()
{
	...
	for (UINT p = 0; p < techDesc.Passes; ++p)
	{
		...
		tessSmapTech->GetPassByIndex(p)->Apply(0, m_pImmediateContext);
		m_pImmediateContext->DrawIndexed(mGridIndexCount, mGridIndexOffset, mGridVertexOffset);
